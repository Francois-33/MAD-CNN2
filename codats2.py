...
class CoDATS(nn.Module):
    ...
    def train_epoch(self):
        ...
        for iteration, data in enumerate(self.trainDataLoader):
            ...
            self.optimizer.step()
        for iteration, data in enumerate(self.testDataLoader):
            self.optimizer.zero_grad()
            inputs_test, domain_test = data
            ...
            loss_domain_test = self.domainLoss(out_dom_test, domain_test)
            # loss_domain = loss_domain + loss_domain_test
            # loss = loss + loss_domain
            loss_domain_test.backward()
            self.optimizer.step()
...
if __name__ == '__main__':
    ...
    CNN_mod = CoDATS(traindata=Datatrain, testdata=Datatest, batchsize=100, name="Codats/Codats_perhaps")
