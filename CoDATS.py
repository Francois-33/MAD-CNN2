...
class CoDATS(nn.Module):
    ...
    def forward_domain(self, train, test=None, train_b=True):
        if test is None:
            ...
            out = self.d(out_conv_train, out_conv_test)
            return out, out_conv_train, out_conv_test

    def train_epoch(self):
        self.train()
        for iteration, data in enumerate(self.trainDataLoader):
            # Optimize for the classification task
            ...
            # Optimize for the domain task on train
            """self.optimizer.zero_grad()
            out_dom, out_conv_dom = self.forward_domain(inputs.transpose(1, 2), train_b=True)
            loss_dom = self.domainLoss(out_dom, domain)
            loss_dom.backward()
            self.optimizer.step()"""

            # self.optimizer.zero_grad()
            inputs_test, labels_test, domain_test = next(iter(self.testDataLoader))
            if torch.cuda.is_available():
                inputs_test = inputs_test.cuda()
                labels_test = labels_test.cuda()
                domain_test = domain_test.cuda()
            self.set_current_batchsize(inputs_test.shape[0], train=False)
            domain_conc = torch.cat((domain, domain_test), 0)
            out_domain, out_conv_train, out_conv_test = self.forward_domain(inputs.transpose(1, 2),
                                                                            inputs_test.transpose(1, 2))
            # out_dom_test, out_conv_dom_test = self.forward_domain(inputs.transpose(1, 2),
            #                                                       inputs_test.transpose(1, 2))
            loss_domain = self.domainLoss(out_domain, domain_conc)
            #print(loss_domain)
            # loss_domain = -1 * loss_domain
            #"print(loss_domain)
        ...
        self.loss_count.append(loss.detach().item() - loss_domain.detach().item())
    ...
    def fit(self, epochs):
        ...
        while self.epoch < epochs:
            ...
            if self.epoch % 1000 == 0:
                ...
                with open(self.name + str(self.epoch) + '.csv', 'w') as file:
                    writer = csv.writer(file, delimiter=' ', quotechar='"')
                    writer.writerows(["Loss classif", self.loss_count])
                    writer.writerows(["Loss domain", self.loss_domain_count])
    ...
    def evaluate_domain(self, test_data):
        self.eval()
        test_data = DataLoader(test_data, batch_size=self.batchsize, shuffle=True)
        with torch.no_grad():
            loss = 0
            ...
            for iteration, data in enumerate(test_data):
                ...
                loss += self.crossLoss(out, target)
                loss += self.domainLoss(out_dom, domain)
        ...
        print('\nAverage loss domain: {:.4f}, Accuracy domain: {}/{} ({:.3f}%)\n'.format(
...
if __name__ == '__main__':
    ...
    test_dataset_t = torch.from_numpy(test_dataset)
    ...
    test4train_pad = np.zeros(shape=(test4train.shape[0], train_dataset.shape[1], test4train.shape[2]))
    test4train_pad[:, :test4train.shape[1]] = test4train
    # test4train_t = torch.from_numpy(test4train_pad)
    ...
    Datatrain = Data_set(train_dataset_t, train_label_t, train_domain_t)
    Datatest = Data_set(test_dataset_t, test_label_t, test_domain_t)
    Datatesttrain = Data_set(test4train_t, test4train_label_t, test4train_domain_t)
    ...
    CNN_mod = CoDATS(traindata=Datatrain, testdata=Datatest, batchsize=128, name="Codats/Codats_maybe")
